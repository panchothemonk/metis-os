...
pub const DEEPSEEK_BLUE: Color =
    Color::Rgb(METIS_PURPLE_RGB.0, METIS_PURPLE_RGB.1, METIS_PURPLE_RGB.2);
...
#[allow(dead_code)]
pub const DEEPSEEK_NAVY: Color =
    Color::Rgb(METIS_ABYSS_RGB.0, METIS_ABYSS_RGB.1, METIS_ABYSS_RGB.2);
pub const DEEPSEEK_INK: Color = Color::Rgb(METIS_VOID_RGB.0, METIS_VOID_RGB.1, METIS_VOID_RGB.2);
pub const DEEPSEEK_SLATE: Color =
    Color::Rgb(METIS_ABYSS_RGB.0, METIS_ABYSS_RGB.1, METIS_ABYSS_RGB.2);
...
pub const BORDER_COLOR: Color =
    Color::Rgb(METIS_BORDER_RGB.0, METIS_BORDER_RGB.1, METIS_BORDER_RGB.2);
#[allow(dead_code)]
...
pub const SURFACE_ELEVATED: Color =
    Color::Rgb(METIS_DEEP_RGB.0, METIS_DEEP_RGB.1, METIS_DEEP_RGB.2);
...
pub fn rgb_to_ansi256(r: u8, g: u8, b: u8) -> u8 {
    ...
    let r6 = (r * 5 / 255) as u8;
    let g6 = (g * 5 / 255) as u8;
    let b6 = (b * 5 / 255) as u8;
    16 + 36 * r6 + 6 * g6 + b6
}
#[must_use]
pub fn nearest_ansi16(r: u8, g: u8, b: u8) -> Color {
    ...
    let sr = if r > 127 { 1 } else { 0 };
    let sg = if g > 127 { 1 } else { 0 };
    let sb = if b > 127 { 1 } else { 0 };
    match (sr, sg, sb, bright) {
        (0, 0, 0, false) => Color::Black,
        (0, 0, 0, true) => Color::Gray,
        (1, 0, 0, false) => Color::Red,
        (1, 0, 0, true) => Color::LightRed,
        (0, 1, 0, false) => Color::Green,
        (0, 1, 0, true) => Color::LightGreen,
        (1, 1, 0, false) => Color::Yellow,
        (1, 1, 0, true) => Color::LightYellow,
        (0, 0, 1, false) => Color::Blue,
        (0, 0, 1, true) => Color::LightBlue,
        (1, 0, 1, false) => Color::Magenta,
        (1, 0, 1, true) => Color::LightMagenta,
        (0, 1, 1, false) => Color::Cyan,
        (0, 1, 1, true) => Color::LightCyan,
        (1, 1, 1, false) => Color::White,
        (1, 1, 1, true) => Color::White,
        _ => {
            if bright {
                Color::Gray
            } else {
